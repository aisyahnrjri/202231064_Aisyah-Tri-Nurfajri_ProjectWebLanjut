            ...
            <div class="card mb-4">
                <div class="card-header pb-0">
                    <div class="d-flex flex-row justify-content-between">
                        <div>
                            <h5 class="mb-0">Pengaturan Pelangggan</h5>
                            <form action="{{route('pelanggan-management.index')}}" class="my-3" method="get">
                        ...
                        <div class="d-flex align-items-start">
                            @include('dashboard.pelanggan-management.tambah.tambah-pelanggan-management')
                        </div>
                    </div>
                </div>
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td class="d-flex justify-content-center gap-2">
                                        <a href="javascript:" data-id="{{ $pelanggan->id_pelanggan }}" onclick="editPelanggan(this)" class="btn bg-gradient-secondary btn-sm m-0">
                                            Ubah
                                        </a>
                                        <a href="{{ url('pengaturan-pelanggan/delete/' . $pelanggan->id_pelanggan) }}" class="btn bg-gradient-danger btn-sm m-0" onclick="event.preventDefault(); confirmDelete(this)">Hapus</a>
                                    </td>
                                </tr>
                                @endforeach
                            </tbody>
                        </table>
                        @include('dashboard.pelanggan-management.ubah.ubah-pelanggan-management')
                        <div class="px-3 pt-3">
...
<script>
    function editPelanggan(button) {
        ...
        $.ajax({
            type: "GET",
            url: "{{url('pengaturan-pelanggan/put')}}/" + pelangganId,
            success: function(ambil) {
                $('#id_pelanggan').val(ambil.id_pelanggan);
                $('#nama_pelanggan').val(ambil.nama_pelanggan);
                $('#username').val(ambil.username);
                $('#nomor_kwh').val(ambil.nomor_kwh);
